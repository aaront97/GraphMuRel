...
class MurelNet(nn.Module):
    ...
    def forward(self, item):
        ...
        question_attentioned_repeated = question_attentioned.unsqueeze(1).expand(-1, num_obj, -1).contiguous()
        
        #Reshape question to (BATCH_SIZE * NUM_OBJ x QUES_DIM)
        question_attentioned_repeated = question_attentioned_repeated.view(batch_size * num_obj, -1)
        
        
        for i in range(self.unroll_steps):
            object_features_list = self.murel_cell(question_attentioned_repeated ,\
