...
def get_dirs(config, include_keys=[]):
    if not include_keys:
        raise ValueError('Please include keys to include\
                          for naming model and log directories')
    root_dir = config['ROOT_DIR']
    model_name = config['name']
    for key in include_keys:
        model_name += "_{}_{}".format(key, config[key])
    log_dir = os.path.join(root_dir, 'logs', model_name)
    checkpoint_dir = os.path.join(root_dir, 'trained_models',
                                  'checkpoints', model_name)
    best_model_dir = os.path.join(root_dir, 'trained_models',
...
# Fix dirs
def run():
    with open('attention_baseline') as f:
        config = yaml.load(f)
    ROOT_DIR = config['ROOT_DIR']
    names = get_dirs(config, include_keys=['txt_enc', 'batch_size', 'lr',
                                           'lr_decay_rate', 'hidden_list',
    ...
    max_accuracy = -1
    if config['checkpoint_option'] == 'resume_last'\
        and (os.path.exists(best_model_file_name)
        or os.path.exists(checkpoint_file_name)):
